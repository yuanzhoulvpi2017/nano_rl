...
def compute_reward_metrics(data: List[Dict], prefix: str = ""):
    score_detail = data

    score_detail_df = pd.DataFrame.from_dict(score_detail)
    metrics = {}
    ...
    for name, func in zip(
        ...
        stat_data = func(score_detail_df, axis=0).to_dict()
        stat_data = {f"{first_tag}/{k}/{name}": v for k, v in stat_data.items()}
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            reward_tensor, score_detail = self.val_reward_fn(test_batch)
        ...
        metric_dict.update(compute_reward_metrics(score_detail, prefix="val"))
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        reward_tensor, score_detail = self.reward_fn(batch)
                        # print(score_detail)
                        batch.batch["token_level_scores"] = reward_tensor
                        batch.batch["score_detail"] = score_detail
                ...
                metrics.update(
                    compute_reward_metrics(data=batch.batch["score_detail"].tolist())
                )
                metrics.update(
